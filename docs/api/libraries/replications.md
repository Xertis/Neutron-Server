...
> [!NOTE]
> Если клиент, которому принадлежит приватная реплика, отключается от сервера, все приватные реплики, привязанные к этому клиенту, автоматически удаляются. Дополнительно вызывать `remove_replica` для них не требуется.
