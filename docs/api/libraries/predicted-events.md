...
> [!WARNING]
> `Instant:interrupt()` отправляет `c_interrupt` только если `instant.active == true`. Для собственного действия это значит — не раньше, чем придёт `on_ack_start`; для наблюдаемого чужого действия `active` не выставляется вовсе.
