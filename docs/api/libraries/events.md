...
```lua
-- Отправляет событие event с данными bytes моду pack на сторону указанного клиента
...
Регистрирует функцию, которая будет вызвана при получении события `event` от мода `pack`. В функцию передаются `Client` и данные `bytes`.
