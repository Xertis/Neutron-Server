...
- `original_packet` — оригинальные данные пакета (не изменяются, доступны только для сравнения/чтения).
- `edited_packet` — данные, изменённые предыдущими обработчиками.

> [!IMPORTANT]
> `edited_packet` — сам пакет, который дальше пойдёт по цепочке.
Если внутри interceptor'а изменить поля `edited_packet`, именно эти изменённые
значения будут переданы следующим interceptor'ам, а в конце — отправлены клиенту.
`original_packet` при этом остаётся нетронутым и служит только для сравнения
"было / стало" — как в примере с `send.add_interceptor` ниже.

**Возвращаемое значение:**
- `true` — продолжить обработку/отправку.
- `false` / `nil` — остановить (пакет не будет отправлен/обработан дальше).
